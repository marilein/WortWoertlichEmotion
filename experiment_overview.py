...
def participants_overview():
    ...
    df_participants = pd.DataFrame(columns=['Kultur', 'count_M', 'count_F', 'count_all', 'mean_age_all', 'min_age_all',
                                            'max_age_all', 'SD_age_all', 'M_mean_age', 'M_SD_age', 'F_mean_age',
                                            'F_SD_age'])
    ...
    for f in file_list:
        ...
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'count_all'] = experiment_overview.shape[
            0]
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'mean_age_all'] = experiment_overview[
            'age'].mean()
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'min_age_all'] = experiment_overview[
            'age'].min()
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'max_age_all'] = experiment_overview[
            'age'].max()
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'SD_age_all'] = np.std(
            experiment_overview['age'].to_list())
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'M_mean_age'] = df_m['age'].mean()
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'M_SD_age'] = np.std(
            df_m['age'].to_list())
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'F_mean_age'] = df_f['age'].mean()
        df_participants.loc[df_participants['Kultur'] == experiment_language, 'F_SD_age'] = np.std(
            df_f['age'].to_list())
